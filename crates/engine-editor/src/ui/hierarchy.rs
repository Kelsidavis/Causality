            ...
            ScrollArea::vertical().show(ui, |ui| {
                ...
                for entity in scene.entities() {
                    if entity.parent.is_none() {
                        render_entity_tree(ui, scene, entity.id, selected_entity, state, hidden_entities, locked_entities, &mut action, 0);
                    }
                }
            });
...
fn render_entity_tree(
    ...
    let is_locked = locked_entities.contains(&entity_id);
    ...
    ui.horizontal(|ui| {
        ...
        let lock_color = if is_locked { egui::Color32::LIGHT_RED } else { egui::Color32::GRAY };
        let lock_btn = ui.add(egui::Button::new(egui::RichText::new(lock_icon).color(lock_color)).frame(false).min_size(egui::vec2(16.0, 16.0)));
        if lock_btn.clicked() {
            action.toggle_lock = Some(entity_id);
        }
        lock_btn.on_hover_text(if is_locked { "Unlock entity" } else { "Lock entity" });
    ...
    if has_children && (is_expanded || !search_filter.is_empty()) {
        for child_id in children {
            render_entity_tree(ui, scene, child_id, selected_entity, state, hidden_entities, locked_entities, action, depth + 1);
        }
    }
}
